...
securityDevicesRouter.get('/',
    ...
    async (req: Request, res: Response) => {
        ...
        if (!devices) {
            res.sendStatus(httpStatusCode.UNAUTHORIZED_401).json({error: "Unauthorized"});
        }
...
securityDevicesRouter.delete('/',
    ...
    async (req: Request, res: Response) => {
        const refreshToken = req.cookies.refreshToken;
        await securityDevicesService.terminateAllSessionExcludeCurrent(refreshToken)
        res.status(httpStatusCode.NO_CONTENT_204).json({message: "Delete all devices"});
    })
...
securityDevicesRouter.delete('/:deviceId',
    ...
    async (req: Request, res: Response) => {
        ...
        if (result.status === resultStatus.NOT_FOUND) {
            res.status(httpStatusCode.NOT_FOUND_404).json({errorsMessages: result.extensions})
            return
        }
