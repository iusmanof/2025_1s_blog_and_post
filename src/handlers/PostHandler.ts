...
const PostHandler = {
    ...
    PUT: async (
        ...
        const postIsUpdated = await postDataAccessLayerMongoDB.updatePost(
            req.params.id,
            req.body,
        );
        const apiErrorMsg: FieldError[] = [];
    ...
    DELETE: async (req: RequestWithParams<{ id: string }>, res: Response) => {
        const post = await postDataAccessLayerMongoDB.deletePost(req.params.id);
